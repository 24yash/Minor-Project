...
def get_relevant_courses_from_cluster_nptel(query, dataframe=df_nptel, top_n=5):
    ...
    return cluster_courses[['Course Name', 'NPTEL URL', 'Course ID', 'Discipline', 'Course Name', 'SME Name', 'Institute', 'Duration']].head(top_n)
...
def get_relevant_courses_from_cluster_mit(query, dataframe=df_mit, top_n=5):
    ...
    cluster_courses['similarity'] = similarity_scores
    cluster_courses = cluster_courses.sort_values(by='similarity', ascending=False)
    return cluster_courses[['Course Title', 'URL', 'Course Number', 'description']].head(top_n)
...
def get_relevant_courses_from_cluster_udemy(query, dataframe=df_udemy, top_n=5):
    ...
    cluster_courses['similarity'] = similarity_scores
    cluster_courses = cluster_courses.sort_values(by='similarity', ascending=False)
    return cluster_courses[['title', 'course_url', 'is_paid', 'headline', 'avg_rating', 'category', 'subcategory', 'topic', 'language', 'course_url', 'instructor_name']].head(top_n)
...
def get_relevant_courses_from_cluster_coursera(query, dataframe=df_coursera, top_n=5):
    ...
    cluster_courses['similarity'] = similarity_scores
    cluster_courses = cluster_courses.sort_values(by='similarity', ascending=False)
    return cluster_courses[['Course Title', 'Course Url', 'Rating', 'Level', 'Schedule', 'Instructor', 'Offered By', 'Keyword', 'Course Url', 'Duration to complete (Approx.)']].head(top_n)
...
if __name__ == "__main__":
    ...
    print(relevant_courses_coursera)